...
public:
	...
	auto setup( std::string name, void* our_function, int arg_count = -1 ) -> bool
	{
		...
		fungi::crt::memory::memcpy( old_instructions, reinterpret_cast< void* >( jit_func_ptr ), hook_size );
		...
		fungi::crt::memory::memcpy( new_instructions, reinterpret_cast< void* >( jit_func_ptr ), hook_size );
	...
	auto setup( std::uintptr_t target_function, void* our_function ) -> bool
	{
		...
		fungi::crt::memory::memcpy( old_instructions, reinterpret_cast< void* >( target_function ), hook_size );
		...
		new_instructions = new std::uint8_t[ hook_size ];
		fungi::crt::memory::memcpy( new_instructions, reinterpret_cast< void* >( target_function ), hook_size );
	...
	{
		if ( !this->m_original || !this->new_instructions )
			return false;
		...
		fungi::crt::memory::memcpy( jit_func_ptr, instructions, hook_size );
