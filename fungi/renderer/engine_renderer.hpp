#pragma once
#include "../sdk/mono/mono_lib.hpp"
#include "../sdk/unity/unity.hpp"
#include "../utility/math.hpp"
#include "../memory/memory.hpp"
#include "../bundle/bundle_data.hpp"
...
namespace renderer
{
	...
	public:
		...
		static math::vec3_t col_to_hsv( color_t a )
		{
			float red, grn, blu;
			red = ( float )a.r / 255.f;
			grn = ( float )a.g / 255.f;
		...
		static math::vec3_t to_hsv( color_t col )
		{
#define SWAP_VARS(type, x, y)  type tmp = x; x = y; y = tmp;
	...
	public:
		...
		{
			MONO_METHOD( get_current_event_fn, E( "UnityEngine::Event.get_current()" ), 0, renderer::c_events * ( * )( ) );
			return get_current_event_fn( );
		}

		auto type( ) -> renderer::event_type
		{
			MONO_METHOD( get_type_fn, E( "UnityEngine::Event.get_type()" ), 0, renderer::event_type( * )( void* ) );
	...
	namespace gui // represents UnityEngine.GUI class
	{
		...
		inline auto setup( ) -> void
		{
			...
			MONO_METHOD( set_color_fn, E( "UnityEngine::GUI.set_color()" ), -1, void( * )( renderer::color_t ) );
		...
		inline auto filled_rect( renderer::rect_t pos, renderer::color_t clr ) -> void
		{
			if ( !fungi::valid_pointer( ( void* )white_texture ) )
				return;

			MONO_METHOD( set_color_fn, E( "UnityEngine::GUI.set_color()" ), -1, void( * )( renderer::color_t ) );
			MONO_METHOD( draw_texture_fn, E( "UnityEngine::GUI.DrawTexture()" ), 2, void ( * )( renderer::rect_t, std::uintptr_t ) );

			set_color_fn( clr );
			draw_texture_fn( pos, white_texture );
		}

		inline auto calculate_text_size( unity::c_string text ) -> math::vec2_t
		{
			if ( !fungi::valid_pointer( ( void* )label_style ) )
				return {};

			MONO_METHOD( create_temp_content_fn, E( "UnityEngine::GUIContent.Temp()" ), -1, std::uintptr_t( * )( unity::c_string* ) );

			const static auto unity_base = fungi::get_module( E( L"UnityPlayer.dll" ) ).base;
			const static auto calc_size_fn = reinterpret_cast< void( __fastcall* )( std::uintptr_t, std::uintptr_t, math::vec2_t& ) >( std::uintptr_t( unity_base ) + 0x3B6E40 );
		...
		inline auto string_with_background( float x, float y, float w, float h, unity::c_string text, renderer::color_t clr, bool drop_shadow ) -> void
		{
			if ( !fungi::valid_pointer( ( void* )label_style ) )
				return;
			...
			MONO_METHOD_ARGS( label_fn, E( "UnityEngine::GUI.Label()" ), 3, E( "content" ), void( * )( renderer::rect_t, std::uintptr_t, std::uintptr_t ) );
		...
		inline auto string( float x, float y, float w, float h, unity::c_string text, renderer::color_t clr, bool centered, bool outlined ) -> void
		{
			if ( !fungi::valid_pointer( label_style ) )
				return;
		...
		inline auto rect( float x, float y, float w, float h, renderer::color_t clr ) -> void
		{
			if ( !fungi::valid_pointer( ( void* )white_texture ) )
				return;
	...
	namespace gl
	{
		...
		inline auto setup( ) -> void
		{
			...
			MONO_METHOD( find_shader_fn, E( "UnityEngine::Shader.Find()" ), -1, std::uintptr_t( * )( unity::c_string ) );
			MONO_METHOD( create_with_shader_fn, E( "UnityEngine::Material.CreateWithShader()" ), 2, void( * )( void*, std::uintptr_t ) );

			renderer::gl::draw_material = *reinterpret_cast< unity::c_material** >( mono_lib::object_new( mono_lib::get_root_domain( ), mono_lib::find_class( E( "Material" ), E( "UnityEngine" ) ) ) );
			create_with_shader_fn( renderer::gl::draw_material, find_shader_fn( E( L"Hidden/Internal-Colored" ) ) );
		...
		inline auto circle( float x, float y, float radius, renderer::color_t col ) -> void
		{
			...
			const static auto color_fn = reinterpret_cast< void( * )( renderer::color_t ) > ( mono_lib::method( E( "UnityEngine::GL.Color()" ), 1 ) );
			if ( !fungi::valid_pointer( color_fn ) )
				return;

			const static auto vertex_fn = reinterpret_cast< void( * )( float, float, float ) >( mono_lib::method( E( "UnityEngine::GL.Vertex3()" ), -1 ) );
			if ( !fungi::valid_pointer( vertex_fn ) )
				return;
		...
		inline auto in_world_line( math::vec3_t origin, math::vec3_t end, renderer::color_t col ) -> void
		{
			if ( !renderer::gl::draw_material )
				return;
			...
			const static auto push_fn = reinterpret_cast< void( * )( void ) >( mono_lib::method( E( "UnityEngine::GL.PushMatrix()" ), -1 ) );
			const static auto begin_fn = reinterpret_cast< void( * )( int ) >( mono_lib::method( E( "UnityEngine::GL.Begin()" ), 1 ) );
			const static auto load_proj_matrix = reinterpret_cast< void( * )( math::mat4x4_t ) >( mono_lib::method( E( "UnityEngine::GL.LoadProjectionMatrix()" ), -1 ) );
			const static auto modelview = reinterpret_cast< void( * )( math::mat4x4_t ) >( mono_lib::method( E( "UnityEngine::GL.set_modelview()" ), -1 ) );
		...
		inline auto line( float x, float y, float x1, float y1, renderer::color_t col ) -> void
		{
		...
		inline auto gradient_filled_rect( float x, float y, float w, float h, renderer::color_t col, renderer::color_t  col2 ) -> void
		{
		...
		inline auto filled_rect( float x, float y, float w, float h, renderer::color_t col ) -> void
		{
			gl_start( gl_primitive_mode::quads );
		...
		inline auto gradient_horizontal_filled_rect( float x, float y, float w, float h, renderer::color_t col, renderer::color_t  col2 ) -> void
		{
			gl_start( gl_primitive_mode::quads );
		...
		inline auto rect( float x, float y, float w, float h, renderer::color_t col ) -> void
		{
